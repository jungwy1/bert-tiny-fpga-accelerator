...
구조 자체는 표준 BERT(Encoder-only Transformer)와 동일하고 크기만 축소되었다.
...
### 2.3 Pooler + Classifier (분류 head)

| 텐서 | shape | 파라미터 |
|------|-------|----------|
| pooler.dense (W+b) + tanh | 128→128 | 16,512 |
| classifier (W+b), fine-tuning 신규 | 128→2 | 258 |
...
| **GRAND TOTAL** | **4,386,178 (≈ 4.39M)** |

- 체크포인트에 함께 들어있는 pretraining head(MLM/NSP)는 **분류 모델 로드 시 폐기**되므로 제외. (상세 §9.3)
...
- 데이터: `datasets.load_dataset("stanfordnlp/sst2")` (train ~67k, validation 872).
- 모델은 **영어 uncased(vocab 30522)** → 영어 데이터 전용. 한국어 불가(→ [UNK]).
- ✅ **FP32 baseline 확보: validation accuracy = 0.8142** — 과정은 [`finetune.md`](finetune.md) 참조.
...
- [x] SST-2로 fine-tuning → **FP32 baseline = 0.8142** ([`finetune.md`](finetune.md))
